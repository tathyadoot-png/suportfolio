    ...
    >
      ...
      >
        ...
        {socials.map((item, idx) => (
          ...
          >
            {/* BJP Orange Glow on Hover */}
            <div 
              className="absolute inset-0 rounded-full md:rounded-2xl opacity-0 group-hover:opacity-100 transition-all duration-500"
              style={{ 
                backgroundColor: "#E46B2E", // Orange hover theme
                boxShadow: `0 10px 20px -5px rgba(228, 107, 46, 0.4)` 
              }}
            />
            
            <item.icon 
              className="w-4 h-4 md:w-5 md:h-5 relative z-10 transition-colors duration-500 group-hover:text-white" 
              style={{ color: item.color }} 
            />
